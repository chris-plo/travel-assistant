...
class TravelApp extends HTMLElement {
  constructor() {
    ...
    this._tripData = null;   // full trip with legs expanded
    this._aiProvider = "none";
    this._view = "itinerary"; // "itinerary" | "trip-settings"
    this._loading = false;
    this._error = null;
  }
  ...
  async _bootstrap() {
    ...
    this._render();
    try {
      // Fetch AI provider from backend config endpoint
  ...
  _render() {
    ...
    <style>
      ...
      details summary{cursor:pointer;font-size:13px;font-weight:600;color:#555;list-style:none;display:flex;align-items:center;gap:6px}
  ...
  _renderAddLegForm() {
    ...
    <div class="leg-form" id="leg-form">
      ...
      <input id="leg-notes"       type="text"           placeholder="Notes">
    </div>
    <div class="leg-form-actions">
      <button class="btn btn-primary" id="btn-add-leg">Add Leg</button>
